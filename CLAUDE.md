...
16. ✅ "Hide conversation" — per-user conversation hiding with auto-unhide on new incoming message; sidebar eye icon toggles between active and hidden views; unhide button restores hidden conversations
...
30. ✅ Responsive CSS — mobile-first breakpoints across all pages; StatusBar mobile overflow "More" menu (⋮); touch-friendly message action buttons; stacked moderation/settings layouts on phones
31. ✅ Sensory prefs cross-user bug fix — `SensoryProvider` embeds `userId` in localStorage cache; rejects mismatched cache; resets visuals instantly on account switch; no more cross-user colour bleed
32. ✅ Minor↔Adult messaging security — bidirectional block in `startConversation` + `sendMessage`; `parent_managed` accounts excluded from user directory for all non-managed requesters; moderator accounts also excluded from directory
33. ✅ Moderator account type — `AccountType.moderator` enum added (Prisma migration); `isModerator` flag on `GET /users/me`; moderators redirect to `/moderation` on login; bypass onboarding gates; moderation link in StatusBar restricted to moderators + pending badge count (60s polling + Socket.io `moderation:new_item` event); moderator test account `mod@test.com` in seed
...
36. ✅ Session focus timer — `useSessionTimer` hook with `SessionTimerProvider` context (root layout); timer starts immediately when user picks a duration (preset buttons: 15/20/25/30 min); `startedAt` timestamp stored in localStorage — survives page navigation; `SessionTimerBar` below StatusBar shows preset buttons when idle, countdown when running with colour states (blue → amber@5min → red@1min); `SessionBreakScreen` soft overlay at 0 (never hard-locks); after dismissing break, timer resets to Off; `GlobalSessionTimer` component in root layout ensures timer visible on all pages
...
44. ✅ Beta testing bug fixes — (a) Email verification bypassed for beta (auto-sets `emailVerified: true` + `status: 'active'` on registration; revert post-beta once Resend domain verified); (b) New conversations appear in real-time (when `message:new` arrives for unknown conversationId, fetches full conversation via API and prepends to list); (c) Typing indicators suppressed during calm mode (gateway checks `userState.calmModeActive` before broadcasting); (d) Queued message status updates for sender (gateway emits `message:status_update` when queued messages are delivered)
45. ✅ Session timer redesign — Replaced dropdown with prominent preset buttons (15/20/25/30 min); timer moved from dashboard to root layout via `SessionTimerProvider` context + `GlobalSessionTimer` component; `startedAt` timestamp in localStorage for persistence across page navigation; after break screen dismissed, resets to Off so user consciously picks new session
46. ✅ UI/UX overhaul (readability + discoverability) — (a) `globals.css`: base `font-size: 16px`, `font-size: inherit` on interactive elements, min-height 2rem on buttons/links; (b) StatusBar: display name is plain text (not a link), explicit gear icon Settings link, bigger energy dots (h-8 w-8), larger padding/spacing throughout; (c) ChatView: prominent blue "How to talk to" button with icon + text label (replaces tiny info circle), larger header icons (h-5 w-5), bigger message input (44px min-height); (d) MessageBubble: "..." dropdown menu replaces crowded inline hover links for delete/report actions, with click-outside and Escape handling
47. ✅ Settings page UX overhaul — Accordion layout (collapsible sections with chevron toggle, dirty badge on unsaved sections); sticky sidebar navigation on desktop (lg+) with jump links and dirty indicators; "Save All" green button in sticky header (appears when dirty sections exist, shows count); custom unsaved-changes modal with 3 options: "Save & leave" / "Discard & leave" / "Stay on this page" (replaces browser `confirm()` for back button and dashboard link); wider max-width (max-w-5xl)
48. ✅ Tester guide rewrite — Both EN (`TESTER_GUIDE.md`) and FR (`TESTER_GUIDE_FR.md`) rewritten to focus heavily on registration and app usage flow (Steps 1-3 with substep details); all UI changes reflected (gear icon, blue info button, "..." menu, preset timer, accordion settings); test accounts and moderator/admin sections moved to bottom labeled "(internal)"; warm accessible tone
...
apps/web/
├── app/
│   ├── layout.tsx              # Root layout (AuthProvider + I18nProvider + SensoryProvider + SessionTimerProvider + GlobalSessionTimer)
│   ├── page.tsx                # Landing page (redirects authenticated users to /dashboard)
│   ├── dashboard/page.tsx      # ✅ Dashboard with messaging UI
│   ├── settings/page.tsx       # ✅ Account settings (accordion, sidebar nav, Save All, unsaved modal)
...
│   ├── use-session-timer.ts    # ✅ Session timer hook (countdown, localStorage persistence, startedAt)
│   ├── session-timer-context.tsx # ✅ SessionTimerProvider + useSessionTimerContext() hook
...
│   ├── GlobalSessionTimer.tsx  # ✅ Root-level session timer (renders SessionTimerBar + SessionBreakScreen)
│   ├── onboarding/
...
│   └── dashboard/
│       ├── StatusBar.tsx       # ✅ Energy badge, calm mode toggle, gear icon settings, bigger nav buttons
...
│       ├── ChatView.tsx        # ✅ Message thread + input + prominent info button + block/report
│       ├── MessageBubble.tsx   # ✅ Single message with "..." action menu (delete/report)
...
- **Hide conversation:** `POST /conversations/:id/hide` hides from sidebar, auto-unhides when a new message arrives from the other user. `POST /conversations/:id/unhide` to manually restore. `GET /conversations?hidden=true` lists hidden conversations. Frontend has an eye-icon toggle in the ConversationList header to switch between active and hidden views, with an "Unhide" button on each hidden conversation item.
...
- **Settings page:** Full `/settings` route reuses onboarding step components. Accordion layout with collapsible sections (chevron toggle, amber dirty badge); only Profile section expanded on load. Sticky sidebar navigation on desktop (lg+) with jump links and dirty indicators. "Save All" green button in sticky header when dirty sections exist. Custom unsaved-changes modal with 3 options (Save & leave / Discard & leave / Stay) replaces browser `confirm()`. Each section also has its own Save button. TimeBoundariesEditor for time boundary management. `beforeunload` remains as fallback for tab close.
...
- **Session focus timer:** `useSessionTimer` hook manages countdown state in `SessionTimerProvider` context (root layout). Stores `{ userId, duration, startedAt }` in localStorage — timer survives page navigation by computing remaining time from `startedAt` on remount. `GlobalSessionTimer` component in root layout renders `SessionTimerBar` + `SessionBreakScreen` for authenticated onboarded users. `SessionTimerBar` shows preset duration buttons (15/20/25/30 min) when idle; countdown with colour states when running (blue → amber@5min → red@1min). Timer starts immediately when user picks a duration (no longer tied to message send). After dismissing break screen, resets to Off — user must consciously pick a new session duration.
...
- **UI/UX baseline:** `globals.css` is minimal: `font-size: 15px` on body, `cursor: pointer` on buttons/links (`default` on disabled), sensory/motion accessibility overrides only. No broad element selectors for sizing — component styles are self-contained via Tailwind classes. StatusBar uses Heroicons v2 Cog6Tooth gear icon for settings and admin; `flex-wrap` on header; nav button padding `px-2.5 py-1`; energy dots `h-6 w-6`. Conversation sidebar width `md:w-[360px]`. ChatView has a prominent blue "How to talk to" button with text label. MessageBubble uses a "..." dropdown menu for delete/report actions instead of inline hover links.
